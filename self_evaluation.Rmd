...
## Linear Regression for Work_done (all_dat)
...
```
## Residual Plot for Work_done (all_dat)
...
## Linear Regression Work_done (morning_rt)
...
all_morning_rt$pred_work_finished <- predict(work_finished_morning, newdata = all_morning_rt)
...
############################# Logistic Regression ##############################
# Logistic Regression For Categorical variable

model <- glm(work_finished ~ Multiple + Phone_pickups + Screen_time + 
                     Drink + Meditation + Reading + Rise_time + Run + 
                     Total_todo,
                   data = all_dat, family = binomial(link = "logit"))
...
ame_result
# How to interpret the AME values
# -> Percentage point rather than percentage
#   -> GESTREC3< 37 weeks  0.0450 : Indicates that if babies are
#      prematurely born, they are 4.5% point more likely to be at Risk.



# We can average the marginal effects across the training data 
...
all_dat$log_pred_work_finished <- predict(model, newdata=all_dat, type = "response")

#ggplot()+geom_density(data= filter(train, atRisk==TRUE), aes(x = pred, color = ’TRUE’))+
#  geom_density(data = filter(train, atRisk == FALSE), aes(x = pred , vcolor = ’FALSE ’ ) )+
#  scale_color_discrete ( " atRisk " )
...
################################# Time Series ##################################
work_finished <- ts(all_dat$work_finished, frequency=365, start=c(2020,9))
plot.ts(work_finished)

total_diff <- ts(all_dat$Tot_diff, frequency=365, start=c(2020,9))
plot.ts(total_diff)
