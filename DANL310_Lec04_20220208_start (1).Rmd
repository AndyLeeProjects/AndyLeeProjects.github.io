...
```{r}
library(gss_sm)
glimpse(gss_sm) # survey data - Nation
...
p <- ggplot(data = gss_sm,
            mapping = aes(x = religion, fill = religion)) # instead of color = religion, fill religion works better
p + geom_bar() + guides(fill = "none") # removes the legend

```

```{r}
# Frequency plots the lightly awkward way
p <- ggplot(data = gss_sm,
            mapping = aes(x=bigregion, fill = religion))
p + geom_bar() # Hard to compare with one another
p + geom_bar(position = 'fill') # Easier this way since it demonstrates the proportion

p + geom_bar(position = "dodge",
             mapping = aes(y=..prop.., group = religion))

# Religion throughout different regions (facet wrap)
p <- ggplot(data = gss_sm,
...
p <- ggplot(data = midwest,
            mapping = aes(x = area))
p + geom_histogram(bins = 10, fill = 'orange') # Always set the number of bins
p + geom_density()
...
p + geom_density(alpha = 0.3) # original
p + geom_density(alpha = 0.3, mapping = (aes(y = ..scaled..))) # adds scale 
```
...
p + geom_bar(position = "dodge", stat = "identity") + theme(legend.position = "top")
   # stat = identity: It means not to change the actual value
```
```{r}
# OECD Data
...
p + geom_col() + guides( fill = "none" ) + 
  ...
  # Default for geom_col: stat = identity
```
