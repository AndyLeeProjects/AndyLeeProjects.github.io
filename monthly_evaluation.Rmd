...
g2 <- ggplot() +
    geom_density(data = filter(all_dat, Reading < 50), 
                 aes(x = Reading, fill = "All_dat"), se = F, alpha = .15) +
    geom_density(data = filter(all_dat, Reading < 50 
                               & month(mdy(Date)) == pprevious_month 
                               & year(mdy(Date))==cur_year), 
                 aes(x = Reading, fill = paste0(month.abb[pprevious_month], '_dat')), 
                 se = F, alpha = .15) +
    geom_density(data = month_dat,
                 aes(x = Reading, fill = paste0(month.abb[previous_month], '_dat')), 
                 se = F, alpha = .6) +
    scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
    labs(title = "Reading (min) Distributions",
             x = "Reading (min)", y = "Percentage (%)") 
...
g3 <- ggplot(month_dat) +
     ...
     labs(title = "\nRise Time Distribution", 
          subtitle = "0: Intended Time(IT), -n: Earlier than IT, +n: Later than IT\n",
          y = "Rise Time (min)")
