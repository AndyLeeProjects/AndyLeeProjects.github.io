...
```{r, echo = FALSE, eval = TRUE, fig.width = 10, fig.height = 8.5,  warning = FALSE, message=FALSE}
library(rstatix)
...
g1 <- ggplot(filter(summary.stats, variable %in% grading & !variable %in% multiple)) +
        ...
        ylim(0,100) + 
        labs(title = "Total & Work Finished Comparison",
             subtitle = "Demonstrates Productivity Level & Overall Percentage\n")+
        scale_fill_manual(values = c("#FBBC97", "#BEC5D6"))

g2 <- ggplot(filter(summary.stats, !variable %in% grading & !variable %in% multiple &
                      variable != "Screen_time")) +
        geom_col(aes(x = month, y = mean, group = variable, fill = variable), position = "dodge") +
        labs(title = "Meditation, Multiple, Reading Comparison",
...
g4 <- ggplot(filter(summary.stats, variable %in% multiple)) +
        geom_smooth(aes(x = month, y = mean, group = variable, color = variable), position = "dodge") +
        labs(title = "Subjective Evaluation - Line",
             subtitle = "1: Worst\n5: Best\n")

g5 <- ggplot(filter(summary.stats, variable == "Screen_time")) +
        geom_smooth(aes(x = month, y = mean, group = variable, color = variable)) +
