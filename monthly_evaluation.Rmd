...
![](`r paste0('/Volumes/Programming/Spring 2022/DANL 310/my_website/aLin-96.github.io/monthly_eval_images/', month.name[previous_month], cur_year)`.jpg)
...
starting_date = paste0(cur_month,'/15/', previous_year)
ending_date = paste0(previous_month, '/30/', cur_year)

dates = seq(as.Date(starting_date, format = "%m/%d/%Y"), 
            by = "month", length.out = 12)
dates_lb = format(seq(as.Date(starting_date, format = "%m/%d/%Y"), 
                      by = "month", length.out = 12), "%m")

ggplot(filter(box_dat, mdy(starting_date) <= mdy(Date) & mdy(Date) < mdy(ending_date))) +
...
### Regression(Linear) Models:Work_finished & Meditation
